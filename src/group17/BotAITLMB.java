...
public class BotAITLMB {
	...
	public ArrayList<Card> findConsecutivePairs(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		if(pre == null) {
			...
			while(j < playerCard.size() && playerCard.get(j).getCost() < 15) {
				if(playerCard.get(i).getCost() == playerCard.get(j).getCost()
						&& playerCard.get(i).checkColorSuit(playerCard.get(j))) {
					if(ans.size() >= 6) {
						if(ans.get(ans.size()-1).getCost() + 1 < playerCard.get(i).getCost()
								|| !ans.get(ans.size()-1).checkColorSuit(playerCard.get(i))) {
					...
					if(ans.size() != 0 && !ans.get(ans.size()-1).checkColorSuit(playerCard.get(i))) {
						ans.clear();
					}
		...
		int i = 0;
		while(playerCard.get(i).getCost() < pre.get(0).getCost() || !playerCard.get(i).checkColorSuit(pre.get(0))) {
			i++;
		...
		while(j < playerCard.size() && playerCard.get(j).getCost() < 15) {
			if(playerCard.get(i).getCost() == playerCard.get(j).getCost()
					&& playerCard.get(i).checkColorSuit(playerCard.get(j)) && playerCard.get(i).checkColorSuit(pre.get(0))) {
	...
	public ArrayList<Card> engineSingle(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		for(int i = 0; i < len; i++) {
			if(playerCard.get(i).compareTo(pre.get(0)) > 0 && playerCard.get(i).getSuit().equals(pre.get(0).getSuit())) {
				ans.add(playerCard.get(i));
				playerCard.remove(i);
				return ans;
			}
			// Neu playerCard[i] la heo thi chat dc moi la le nho hon
			if(playerCard.get(i).compareTo(pre.get(0)) > 0 && playerCard.get(i).getCost() == 15) {
				ans.add(playerCard.get(i));
				playerCard.remove(i);
				return ans;
			}
	...
	public ArrayList<Card> engineDouble(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		int i = 0;
		while(playerCard.get(i).getCost() < pre.get(0).getCost() || !playerCard.get(i).checkColorSuit(pre.get(0))) {
			if(playerCard.get(i).getCost() == 15) {
				break;
			}
			i++;
		...
		while(i < playerCard.size() - 1) {
			if(playerCard.get(i).getCost() == playerCard.get(i+1).getCost()
					&& playerCard.get(i).checkColorSuit(playerCard.get(i+1))) {
				if(playerCard.get(i+1).compareTo(pre.get(1)) > 0 && playerCard.get(i).checkColorSuit(pre.get(0))) {
					ans.add(playerCard.get(i));
					playerCard.remove(i);
					ans.add(playerCard.get(i));
					playerCard.remove(i);
				...
				// Doi heo chat duoc bat ki doi nao khac nho hon ma ko can cung chat
				if(playerCard.get(i).getCost() == 15 && playerCard.get(i+1).getCost() == 15
						&& playerCard.get(i).checkColorSuit(playerCard.get(i+1))) {
	...
	public ArrayList<Card> engineTriple(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		for(int i = 0; i < playerCard.size() - 2; i++) {
			if(playerCard.get(i).getCost() == playerCard.get(i+2).getCost() && playerCard.get(i).getCost() > pre.get(0).getCost()) {
				boolean check = false;
				for(int j = 0; j < 3; j++) {
					if(!playerCard.get(j+i).getSuit().equals(pre.get(j).getSuit())) {
						check = true;
						break;
					}
				}
	...
	public ArrayList<Card> engineSequence(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		int len = pre.size();
		int i = 0;
		while(playerCard.get(i).compareTo(pre.get(0)) < 0 || !playerCard.get(i).getSuit().equals(pre.get(0).getSuit())) {
			i++;
			if(i == playerCard.size() || playerCard.get(i).getCost() == 15) {
				return null;
			}
		...
		while(i < playerCard.size() && playerCard.get(i).getCost() < 15) {
			if(ans.size() == 0 && playerCard.get(i).getSuit().equals(pre.get(0).getSuit())) {
				ans.add(playerCard.get(i));
			}
			else if(ans.size() != 0 && ans.get(ans.size()-1).getCost() + 1 < playerCard.get(i).getCost()) {
				ans.clear();
				if(playerCard.get(i).getSuit().equals(pre.get(0).getSuit())) {
					ans.add(playerCard.get(i));
				}
			}
			else if(ans.size() != 0 && ans.get(ans.size()-1).getCost() + 1 == playerCard.get(i).getCost()) {
