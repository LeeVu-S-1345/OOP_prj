...
public class GameTLMB extends Game implements MouseListener, Runnable{
...
//	private JButton btnHide = new JButton("Hide");
	...
	private BotAITLMB bot = new BotAITLMB();
...
//		panel.add(btnHide);
	...
	public boolean singleCard() {
		...
		if(pre.size() == 1) {
			if(chosenCard.size() == 1 && pre.get(0).getCost() < chosenCard.get(0).getCost()
					&& pre.get(0).getSuit().equals(chosenCard.get(0).getSuit())) {
				return true;
			}
			
			if(chosenCard.size() == 1 && chosenCard.get(0).getCost() == 15
					&& pre.get(0).compareTo(chosenCard.get(0)) < 0) {
				return true;
			}
				...
				// Doi thong
					...
					for(int i = 0; i < chosenCard.size() - 3; i+=2) {
						...
						if(!chosenCard.get(i).checkColorSuit(chosenCard.get(i+1))){
							return false;
						}
						if(!chosenCard.get(i).checkColorSuit(chosenCard.get(i+2))){
							return false;
						}
					}
					...
					if(!chosenCard.get(chosenCard.size()-2).checkColorSuit(chosenCard.get(chosenCard.size()-1))){
						return false;
					}
					return true;
				}
	...
	public boolean doubleCard() {
		if(pre.size() == 0 && chosenCard.size() == 2) {
			if(chosenCard.get(0).getCost() == chosenCard.get(1).getCost()
					&& chosenCard.get(0).checkColorSuit(chosenCard.get(1))) {
				return true;
			}
		}
		if(pre.size() == 2 && chosenCard.size() == 2) {
			if(chosenCard.get(0).getCost() == chosenCard.get(1).getCost()
					&& chosenCard.get(0).checkColorSuit(chosenCard.get(1))) {
				if(pre.get(1).getCost() < chosenCard.get(1).getCost()) {
					return true;
				}
			}
		}
		// Doi 2 khong can cung suit
		if(pre.size() == 2 && chosenCard.get(0).getCost() == 15 && chosenCard.size() == 2) {
			if(chosenCard.get(1).getCost() > pre.get(1).getCost()) {
				return true;
			}
		}
		// tu quy chat doi 2
		...
		if(pre.size() == 2 && chosenCard.size() == 10) {
			if(chosenCard.get(0).getCost() >= 11) {
				return false;
			}
			for(int i = 0; i < 7; i+=2) {
				if(chosenCard.get(i).getCost() != chosenCard.get(i+1).getCost()) {
			...
			if(!chosenCard.get(8).checkColorSuit(chosenCard.get(9))){
				return false;
			}
			return true;
		}
	...
	public boolean tripleCard() {
		...
		if(pre.size() == 3 && chosenCard.size() == 3) {
			...
			if(chosenCard.get(0).getCost() < pre.get(0).getCost()) {
				return false;
			}
			for(int i = 0; i < 3; i++) {
				if(!chosenCard.get(i).getSuit().equals(pre.get(i).getSuit())) {
					return false;
				}
			}
			return true;
	...
	public boolean sequenceCard() {
		...
		if(pre.size() == 0 && size >= 3 && chosenCard.get(size-1).getCost() < 15) {
			for(int i = 0; i < chosenCard.size()-1; i++) {
				...
				if(!chosenCard.get(i).getSuit().equals(chosenCard.get(i+1).getSuit())) {
					return false;
				}
			}
			return true;
		...
		if(pre.size() >= 3 && size == pre.size()) {
			...
			if(pre.get(pre.size()-1).compareTo(chosenCard.get(size-1)) < 0 && chosenCard.get(0).getSuit().equals(pre.get(0).getSuit())) {
				return true;
			}
		}
		return false;
	...
	public boolean consecutivePairs() {
		...
		if(pre.size() == 0 && len >= 6 && len % 2 == 0 && chosenCard.get(len - 1).getCost() < 15) {
			...
			if(!chosenCard.get(len-2).checkColorSuit(chosenCard.get(len-1))){
				return false;
			}
			return true;
		}
		...
		if(pre.size() >= 6 && len >= 6 && len % 2 ==0) {
			...
			if(pre.get(pre.size()-1).compareTo(chosenCard.get(len-1)) < 0 && pre.get(0).getSuit().equals(chosenCard.get(0).getSuit())) {
				return true;
			}
		}
